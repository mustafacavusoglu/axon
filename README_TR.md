...
| Dizin | Model | Platform | Açıklama |
|-------|-------|----------|----------|
| `models/axon_turkish_safety/` | Türkçe güvenlik sınıflandırıcı | ensemble | BERT tabanlı NLP pipeline (tokenizer → ONNX → decoder) |
| `models/cb_credit_risk/` | Kredi riski sınıflandırıcı | onnxruntime_onnx | CatBoost (15 özellik → temerrüt olasılığı) |
| `models/xgb_california_housing/` | Ev fiyatı regresyonu | onnxruntime_onnx | XGBoost (8 özellik → fiyat) |
| `models/skl_random_forest_breast_cancer/` | Kanser teşhis sınıflandırıcı | onnxruntime_onnx | sklearn Random Forest (30 özellik → iyi/kötü huylu) |
| `models/echo_preprocess/` | Echo ön işlemci | script | Girdiyi olduğu gibi geçiren Rhai script |

Her model dizini `models/<isim>/` altında kendi kendine yeterlidir:
```
models/<isim>/
├── config.pbtxt          # Model konfigürasyonu (repoda)
└── 1/
    ├── model.onnx        # ONNX model dosyası (gitignored — dönüştür veya indir)
    ├── model.rhai        # Rhai script (repoda, script/ensemble modeller için)
    ├── tokenizer.json    # HuggingFace tokenizer (gitignored — convert script ile üret)
    ├── tokenizer_config.json
    ├── special_tokens_map.json
    └── vocab.txt
```

> **Not:** Büyük/üretilen dosyalar (`*.onnx`, `tokenizer.json`, `vocab.txt` vb.) gitignored'dur.
> `locals/scripts/` altındaki dönüşüm scriptleriyle üretebilir veya HuggingFace'ten
> indirebilirsiniz.

```bash
# Pipeline inference — metinden güvenlik etiketine
